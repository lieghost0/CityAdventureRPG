...
namespace Framework
{
    public class LuaManager : MonoBehaviour
    {
        ...
        public void Init()
        {
            ...
            LuaEnv.AddBuildin("repinjson", XLua.LuaDLL.Lua.LoadRapidJson);
        ...
        public byte[] GetLuaScript(string name)
        {
            ...
            {
                // --添加日志提示
                LogUtil.Error(string.Format("{0}文件不存在", fileName));
                return null;
            }
            return luaScript;
        }
        ...
        void LoadLuaScript()
        {
            ...
            {
                Manager.Resource.LoadLua(name, (obj) =>
                {
        ...
        private void AddLuaScript(string assetsName, byte[] bytes)
        {
            m_LuaScripts[assetsName] = bytes;
        }

#if UNITY_EDITOR
        void EditorLoadLuaScript()
        {
            string[] luaFiles = Directory.GetFiles(PathUtil.LuaPath, "*.lua.txt", SearchOption.AllDirectories);
